...
export const ResearchStudio = ({ workspace = 'research' }) => {
  ...
  return (
    <div className={`${isPaperWorkspace ? 'paper-editor-page h-full min-h-0 overflow-hidden' : 'min-h-[calc(100vh-64px)]'} bg-[#f4f6fb] text-slate-900 dark:bg-background dark:text-on-surface`}>
      <div className={`w-full px-3 py-3 md:px-4 md:py-4 ${isPaperWorkspace ? 'paper-editor-page__content flex h-full min-h-0 flex-col overflow-hidden' : ''}`}>
        <header className={`mb-2 flex min-h-14 flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-200/80 bg-white px-3 py-2 shadow-sm dark:border-outline-variant/30 dark:bg-surface ${isPaperWorkspace ? 'shrink-0' : ''}`}>
        ...
        <div className={isPaperWorkspace ? 'paper-editor-workbench-shell min-h-0 flex-1 overflow-hidden' : 'min-h-[calc(100vh-178px)]'}>
          ...
          <section ref={selectedDocument?.kind === 'code' ? codeWorkspaceRef : null} className={`min-w-0 overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-sm dark:border-outline-variant/30 dark:bg-surface ${isPaperWorkspace ? 'flex h-full min-h-0 flex-col' : ''} ${isCodeFullscreen ? 'h-screen overflow-y-auto rounded-none' : ''}`}>
