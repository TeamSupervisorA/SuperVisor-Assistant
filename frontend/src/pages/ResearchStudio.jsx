...
const starterForLanguage = (language) => ({
  ...
  python: `# Research implementation notebook\n# Keep experiments reproducible: document inputs, outputs, and assumptions.\n\ndef main():\n    print('Start your research implementation here')\n\nif __name__ == '__main__':\n    main()`,
  java: `public class Main {\n  public static void main(String[] args) {\n    System.out.println("Start your research implementation here");\n  }\n}`,
  cpp: `#include <iostream>\n\nint main() {\n  std::cout << "Start your research implementation here\\n";\n  return 0;\n}`,
  c: `#include <stdio.h>\n\nint main(void) {\n  puts("Start your research implementation here");\n  return 0;\n}`,
  csharp: `using System;\n\nConsole.WriteLine("Start your research implementation here");`,
  go: `package main\n\nimport "fmt"\n\nfunc main() {\n  fmt.Println("Start your research implementation here")\n}`,
  rust: `fn main() {\n    println!("Start your research implementation here");\n}`,
  r: `# Research implementation notebook\nprint("Start your research implementation here")`,
  julia: `# Research implementation notebook\nprintln("Start your research implementation here")`,
  php: `<?php\necho "Start your research implementation here\\n";`,
  ruby: `puts 'Start your research implementation here'`,
  sql: `-- Use a configured SQL runner only with non-production sample data.\nSELECT 'Start your research implementation here' AS message;`,
  bash: `#!/usr/bin/env bash\necho "Start your research implementation here"`
}[language] || codeStarter);
...
const PAPER_SPLIT_STORAGE_KEY = 'supervisor-paper-editor-split-ratio';
const initialPaperSplitRatio = () => {
  try {
    const stored = Number(window.localStorage.getItem(PAPER_SPLIT_STORAGE_KEY));
    return Number.isFinite(stored) && stored >= 28 && stored <= 72 ? stored : 50;
  } catch {
    return 50;
  }
};
...
export const ResearchStudio = ({ workspace = 'research' }) => {
  ...
  const [splitRatio, setSplitRatio] = useState(initialPaperSplitRatio);
  ...
  const [isPaperFocusMode, setIsPaperFocusMode] = useState(false);
  ...
  const paperWorkspaceRef = useRef(null);
  const paperResizePointerRef = useRef(null);
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem(PAPER_SPLIT_STORAGE_KEY, String(splitRatio));
    } catch {
      // Storage is optional; a blocked browser must not affect editing.
    }
  }, [splitRatio]);
  useEffect(() => {
    if (!isResizingPaper) return undefined;
  ...
  useEffect(() => {
    if (!isPaperFocusMode) return undefined;
    const closeOnEscape = (event) => {
      if (event.key === 'Escape' && !event.defaultPrevented) setIsPaperFocusMode(false);
    };
    const closeOnNarrowViewport = () => {
      if (window.matchMedia('(max-width: 1023px)').matches) setIsPaperFocusMode(false);
    };
    document.addEventListener('keydown', closeOnEscape);
    window.addEventListener('resize', closeOnNarrowViewport);
    return () => {
      document.removeEventListener('keydown', closeOnEscape);
      window.removeEventListener('resize', closeOnNarrowViewport);
    };
  }, [isPaperFocusMode]);
  useEffect(() => {
    setCompiledPdf('');
  ...
  const runJavascriptInWorker = useCallback((source) => new Promise((resolve) => {
    const workerSource = `
      ...
      self.onmessage = async ({ data }) => {
        ...
        try {
          ...
          const result = await execute(self.console);
          if (result !== undefined) capture(result);
          self.postMessage({ ok: true, output: lines.join('\\n') || '(Program completed with no output.)' });
        } catch (error) {
          self.postMessage({ ok: false, output: lines.concat(error.name + ': ' + error.message).join('\\n') });
        }
      };
    `;
    ...
    const timeout = window.setTimeout(() => {
      ...
      resolve({ ok: false, output: 'Execution stopped after 4 seconds to protect the browser workspace.' });
    }, 4000);
  ...
  const toggleCodeFullscreen = async () => {
    try {
      ...
      else await codeWorkspaceRef.current?.requestFullscreen?.();
    } catch {
      setError('Fullscreen mode is not available in this browser.');
    }
  };

  const togglePaperFocusMode = () => {
    if (window.matchMedia('(max-width: 1023px)').matches) {
      setError('The full paper workspace is available on desktop-size screens.');
      return;
    }
    setIsPaperFocusMode((current) => !current);
  };
  ...
  const paperSplitBounds = useCallback(() => {
    const width = paperWorkspaceRef.current?.getBoundingClientRect().width || 0;
    // Keep both desktop panes usable after the navigation rail expands. The
    // CSS breakpoint stacks the workspace before this would become cramped.
    const minimum = width ? Math.min(45, Math.max(28, (320 / Math.max(1, width - 14)) * 100)) : 28;
    return { min: minimum, max: 100 - minimum };
  }, []);

  const clampPaperSplit = useCallback((value) => {
    const { min, max } = paperSplitBounds();
    return Math.min(max, Math.max(min, value));
  }, [paperSplitBounds]);

  const setPaperSplitFromPointer = useCallback((clientX) => {
    const bounds = paperWorkspaceRef.current?.getBoundingClientRect();
    if (!bounds?.width) return;
    const usableWidth = Math.max(1, bounds.width - 14);
    const next = ((clientX - bounds.left - 7) / usableWidth) * 100;
    setSplitRatio(clampPaperSplit(next));
  }, [clampPaperSplit]);

  const startPaneResize = (event) => {
    if (event.pointerType === 'mouse' && event.button !== 0) return;
    event.preventDefault();
    paperResizePointerRef.current = event.pointerId;
    event.currentTarget.setPointerCapture?.(event.pointerId);
    setIsResizingPaper(true);
    setPaperSplitFromPointer(event.clientX);
  };

  const movePaneResize = (event) => {
    if (paperResizePointerRef.current !== event.pointerId) return;
    setPaperSplitFromPointer(event.clientX);
  };

  const stopPaneResize = (event) => {
    if (paperResizePointerRef.current !== event.pointerId) return;
    if (event.currentTarget.hasPointerCapture?.(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId);
    paperResizePointerRef.current = null;
    setIsResizingPaper(false);
  };

  const adjustPaneRatio = (amount) => setSplitRatio((current) => clampPaperSplit(current + amount));
  ...
  const handlePaneResizeKeyDown = (event) => {
    if (event.key === 'ArrowLeft') {
      ...
      event.preventDefault();
      setSplitRatio(paperSplitBounds().min);
    } else if (event.key === 'End') {
      event.preventDefault();
      setSplitRatio(paperSplitBounds().max);
  ...
  return (
    <div className={`${isPaperWorkspace ? `paper-editor-page min-h-full ${isPaperFocusMode ? 'paper-editor-page--focus' : ''}` : 'min-h-[calc(100vh-64px)]'} bg-[#f4f6fb] text-slate-900 dark:bg-background dark:text-on-surface`}>
      <div className={`w-full px-3 py-3 md:px-4 md:py-4 ${isPaperWorkspace ? 'paper-editor-page__content flex min-h-full flex-col' : ''}`}>
        ...
        {error && <div role="alert" className="paper-editor-page__alert mb-5 flex items-center justify-between gap-4 rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-800 dark:border-error/30 dark:bg-error-container/30 dark:text-error"><span>{error}</span><button onClick={() => setError('')} aria-label="Dismiss message" className="material-symbols-outlined text-[18px]">close</button></div>}

        {selectedDocument && <div className="paper-editor-page__utility mb-2 flex flex-wrap items-center justify-between gap-2 rounded-lg border border-slate-200/80 bg-white px-2 py-1.5 shadow-sm dark:border-outline-variant/30 dark:bg-surface"><div className="flex min-w-0 items-center gap-1 overflow-x-auto">{selectedDocument.kind === 'paper' && <button onClick={() => setUtilityPanel(utilityPanel === 'literature' ? '' : 'literature')} className={`rounded-md px-2.5 py-1.5 text-xs font-bold ${utilityPanel === 'literature' ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-slate-100 dark:text-secondary dark:hover:bg-surface-container'}`}><span className="material-symbols-outlined mr-1 align-[-3px] text-[15px]">menu_book</span>Literature</button>}<button onClick={() => setUtilityPanel(utilityPanel === 'outline' ? '' : 'outline')} className={`rounded-md px-2.5 py-1.5 text-xs font-bold ${utilityPanel === 'outline' ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-slate-100 dark:text-secondary dark:hover:bg-surface-container'}`}><span className="material-symbols-outlined mr-1 align-[-3px] text-[15px]">account_tree</span>Outline</button><button onClick={() => setUtilityPanel(utilityPanel === 'quality' ? '' : 'quality')} className={`rounded-md px-2.5 py-1.5 text-xs font-bold ${utilityPanel === 'quality' ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-slate-100 dark:text-secondary dark:hover:bg-surface-container'}`}><span className="material-symbols-outlined mr-1 align-[-3px] text-[15px]">fact_check</span>{selectedDocument.kind === 'paper' ? 'Checks' : 'IDE status'}</button>{selectedDocument.kind === 'code' && <button onClick={() => setUtilityPanel(utilityPanel === 'runtime' ? '' : 'runtime')} className={`rounded-md px-2.5 py-1.5 text-xs font-bold ${utilityPanel === 'runtime' ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-slate-100 dark:text-secondary dark:hover:bg-surface-container'}`}><span className="material-symbols-outlined mr-1 align-[-3px] text-[15px]">memory</span>Runtimes</button>}{selectedDocument.kind === 'paper' && <button onClick={() => setUtilityPanel(utilityPanel === 'overleaf' ? '' : 'overleaf')} className={`rounded-md px-2.5 py-1.5 text-xs font-bold ${utilityPanel === 'overleaf' ? 'bg-indigo-600 text-white' : 'text-slate-600 hover:bg-slate-100 dark:text-secondary dark:hover:bg-surface-container'}`}><span className="material-symbols-outlined mr-1 align-[-3px] text-[15px]">open_in_new</span>Overleaf</button>}</div><div className="hidden shrink-0 gap-1.5 text-[11px] font-bold text-slate-500 md:flex dark:text-secondary">{selectedDocument.kind === 'paper' ? <><span className="rounded-full bg-slate-100 px-2 py-1 dark:bg-surface-container">{wordCount.toLocaleString()} words</span><span className="rounded-full bg-slate-100 px-2 py-1 dark:bg-surface-container">{sections.length} sections</span><span className="rounded-full bg-slate-100 px-2 py-1 dark:bg-surface-container">{citationCount} cites</span></> : <><span className="rounded-full bg-slate-100 px-2 py-1 dark:bg-surface-container">{codeStats.lines} lines</span><span className="rounded-full bg-slate-100 px-2 py-1 dark:bg-surface-container">{codeStats.todos} TODOs</span></>}</div></div>}
        {selectedDocument && utilityPanel && utilityPanel !== 'literature' && <div className="paper-editor-page__utility-panel mb-2 max-h-32 overflow-y-auto rounded-lg border border-slate-200 bg-white p-2.5 shadow-sm dark:border-outline-variant/30 dark:bg-surface">{utilityPanel === 'outline' && <div className="flex flex-wrap gap-2">{sections.length ? sections.map((section, index) => <span key={`${section}-${index}`} className="rounded-lg bg-slate-100 px-3 py-2 text-xs font-semibold text-slate-700 dark:bg-surface-container dark:text-on-surface">{index + 1}. {section}</span>) : <span className="text-xs text-slate-500 dark:text-secondary">{selectedDocument.kind === 'paper' ? 'Add \\section headings to create an outline.' : 'Use comments and clear function names to keep the implementation easy to navigate.'}</span>}</div>}{utilityPanel === 'quality' && (selectedDocument.kind === 'paper' ? <div className="flex flex-wrap gap-2">{readiness.map(([label, complete]) => <span key={label} className={`rounded-lg px-3 py-2 text-xs font-semibold ${complete ? 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-300' : 'bg-slate-100 text-slate-500 dark:bg-surface-container dark:text-secondary'}`}>{complete ? '✓' : '○'} {label}</span>)}</div> : <p className="text-xs leading-relaxed text-slate-600 dark:text-secondary">{codeStats.logs} output call(s) and {codeStats.todos} TODO marker(s) detected. Download the source and run it only in an approved development environment.</p>)}{utilityPanel === 'runtime' && <p className="text-xs leading-relaxed text-slate-600 dark:text-secondary">JavaScript uses a time-limited browser worker. Python, Java, C/C++, C#, Go, Rust, R, Julia, PHP, Ruby, SQL, Bash, and TypeScript require your institution’s isolated runner. Available libraries are controlled by that runner’s installed runtime; do not enable unrestricted package installation for untrusted code.</p>}{utilityPanel === 'overleaf' && <div className="flex flex-col gap-2 sm:flex-row"><input value={selectedDocument.overleafUrl || ''} onChange={(event) => updateSelected({ overleafUrl: event.target.value })} placeholder="https://www.overleaf.com/project/…" className="min-w-0 flex-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-xs outline-none focus:border-indigo-400 dark:border-outline-variant/50 dark:bg-surface-container-lowest"/><button onClick={openOverleaf} className="rounded-lg bg-indigo-600 px-4 py-2 text-xs font-bold text-white hover:bg-indigo-700">{selectedDocument.overleafUrl ? 'Open linked project' : 'Create Overleaf project'}</button></div>}</div>}

        {selectedDocument?.kind === 'paper' && utilityPanel === 'literature' && <section className="paper-editor-page__literature mb-2 max-h-[38dvh] shrink-0 overflow-y-auto rounded-lg border border-indigo-200/80 bg-white px-3 py-2.5 shadow-sm dark:border-primary/25 dark:bg-surface"><div className="flex flex-col justify-between gap-2 lg:flex-row lg:items-center"><div className="shrink-0"><div className="flex items-center gap-2"><span className="material-symbols-outlined text-[18px] text-indigo-600 dark:text-primary">menu_book</span><h2 className="text-sm font-extrabold text-slate-900 dark:text-on-surface">Literature search</h2><span className="rounded-full bg-indigo-50 px-2 py-0.5 text-[10px] font-bold text-indigo-700 dark:bg-primary/10 dark:text-primary">OpenAlex + Crossref</span></div></div><form onSubmit={searchLiterature} className="flex w-full max-w-2xl gap-2"><input value={literatureQuery} onChange={(event) => setLiteratureQuery(event.target.value)} placeholder="Search papers, methods, authors, or datasets…" className="min-w-0 flex-1 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none transition focus:border-indigo-400 focus:ring-4 focus:ring-indigo-100 dark:border-outline-variant/50 dark:bg-surface-container-lowest dark:focus:border-primary dark:focus:ring-primary/10"/><button disabled={searching} className="rounded-lg bg-indigo-600 px-4 text-sm font-bold text-white hover:bg-indigo-700 disabled:opacity-60 dark:bg-primary dark:text-on-primary">{searching ? 'Searching…' : 'Search'}</button></form></div>{literature.length > 0 && <div className="mt-2 grid gap-2 border-t border-slate-100 pt-2 dark:border-outline-variant/30">{literature.map((work) => <div key={`${work.source}-${work.id}`} className="flex flex-col justify-between gap-3 rounded-lg bg-slate-50 p-3 dark:bg-surface-container-low sm:flex-row"><div className="min-w-0"><div className="mb-1 flex flex-wrap gap-2"><span className="rounded-full bg-indigo-100 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-indigo-700 dark:bg-primary/15 dark:text-primary">{work.source}</span>{work.year && <span className="text-xs text-slate-500 dark:text-secondary">{work.year}</span>}</div><a href={work.url} target="_blank" rel="noreferrer" className="text-sm font-bold leading-snug text-slate-900 hover:text-indigo-700 hover:underline dark:text-on-surface dark:hover:text-primary">{work.title}</a><p className="mt-1 text-xs leading-relaxed text-slate-500 dark:text-secondary">{work.authors?.join(', ') || 'Authors unavailable'}{work.venue ? ` · ${work.venue}` : ''}{work.citedBy ? ` · ${work.citedBy.toLocaleString()} citations` : ''}</p></div><button type="button" onClick={() => addCitation(work)} className="h-8 shrink-0 rounded-lg border border-indigo-200 px-3 text-xs font-bold text-indigo-700 hover:bg-indigo-50 dark:border-primary/30 dark:text-primary dark:hover:bg-primary/10">Add citation</button></div>)}</div>}<p className="mt-2 text-[11px] text-slate-500 dark:text-secondary">Verify the original source, authors, DOI, and citation details before submission.</p></section>}

        <div className={isPaperWorkspace ? 'paper-editor-workbench-shell min-h-0 flex-1 overflow-hidden' : 'min-h-[calc(100vh-178px)]'}>
          ...
          <section ref={selectedDocument?.kind === 'code' ? codeWorkspaceRef : null} className={`min-w-0 overflow-hidden rounded-xl border border-slate-200/80 bg-white shadow-sm dark:border-outline-variant/30 dark:bg-surface ${isPaperWorkspace ? 'paper-editor-workbench flex h-full min-h-0 flex-col' : ''} ${isCodeFullscreen ? 'h-screen overflow-y-auto rounded-none' : ''}`}>
            {!selectedDocument && loading ? <div className="grid min-h-[680px] place-items-center p-8 text-center" role="status"><div><div className="mx-auto size-10 animate-spin rounded-full border-4 border-indigo-100 border-t-indigo-600 dark:border-primary/20 dark:border-t-primary"/><p className="mt-4 text-sm font-medium text-slate-500 dark:text-secondary">Loading workspace…</p></div></div> : !selectedDocument ? <div className="grid min-h-[680px] place-items-center p-8 text-center"><div><span className="material-symbols-outlined text-[52px] text-indigo-500">edit_note</span><h2 className="mt-4 text-xl font-bold">Your research workspace is ready</h2><p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-slate-500 dark:text-secondary">{isPaperWorkspace ? 'Create a LaTeX paper to start drafting.' : isCodeWorkspace ? 'Create a code file to start the implementation workspace.' : 'Create a paper or code file to start your project record.'}</p><div className="mt-5 flex flex-wrap justify-center gap-2">{!isCodeWorkspace && <button type="button" onClick={() => createDocument('paper')} className="rounded-lg bg-indigo-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-indigo-700">Create paper</button>}{!isPaperWorkspace && <button type="button" onClick={() => createDocument('code')} className="rounded-lg border border-slate-200 bg-white px-4 py-2.5 text-sm font-bold text-slate-700 hover:border-indigo-300 hover:text-indigo-700 dark:border-outline-variant/40 dark:bg-surface dark:text-on-surface">Create code file</button>}</div></div></div> : <>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2 border-b border-slate-200 px-4 py-2.5 dark:border-outline-variant/30"><div className="inline-flex items-center gap-2 text-xs text-slate-500 dark:text-secondary"><span className="material-symbols-outlined text-[16px] text-indigo-600 dark:text-primary">description</span>{selectedDocument.kind === 'paper' ? 'Paper document' : 'Implementation file'} · {dirty ? 'Unsaved changes' : status || relativeTime(selectedDocument.updatedAt)}<button onClick={() => { const title = window.prompt('Rename document', selectedDocument.title)?.trim(); if (title) updateSelected({ title }); }} title="Rename document" className="ml-1 rounded p-1 text-slate-400 hover:bg-slate-100 hover:text-indigo-600 dark:hover:bg-surface-container"><span className="material-symbols-outlined text-[15px]">edit</span></button></div><div className="flex flex-wrap items-center gap-1.5">{selectedDocument.kind === 'paper' && <><select value={compilerEngine} onChange={(event) => setCompilerEngine(event.target.value)} aria-label="LaTeX compiler" className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[11px] font-bold text-slate-700 outline-none focus:border-indigo-400 dark:border-outline-variant/40 dark:bg-surface-container dark:text-on-surface"><option value="pdflatex">pdfLaTeX</option><option value="xelatex">XeLaTeX</option><option value="lualatex">LuaLaTeX</option></select><button onClick={() => compileDocument()} disabled={isCompiling} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-emerald-700 disabled:cursor-wait disabled:opacity-60"><span className="material-symbols-outlined mr-1 align-[-3px] text-[16px]">play_arrow</span>{isCompiling ? 'Compiling…' : compiledPdf ? 'Recompile' : 'Compile'}</button></>}{selectedDocument.kind === 'code' && <><select value={selectedDocument.language || 'javascript'} onChange={(event) => { updateSelected({ language: event.target.value }); setCodeOutput({ text: `${event.target.options[event.target.selectedIndex].text} selected. Load a starter if you want a new template.`, state: 'idle', language: event.target.value }); }} aria-label="Programming language" className="rounded-lg border border-slate-200 bg-white px-2 py-1.5 text-[11px] font-bold text-slate-700 outline-none focus:border-indigo-400 dark:border-outline-variant/40 dark:bg-surface-container dark:text-on-surface">{codeLanguages.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><button onClick={() => { const language = selectedDocument.language || 'javascript'; if (window.confirm(`Replace the current source with a ${language} starter template?`)) updateSelected({ content: starterForLanguage(language) }); }} title="Load language starter" className="grid size-8 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 hover:text-indigo-700 dark:text-secondary dark:hover:bg-surface-container"><span className="material-symbols-outlined text-[18px]">note_add</span></button><button onClick={runCode} disabled={isRunningCode} className="rounded-lg bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white hover:bg-emerald-700 disabled:cursor-wait disabled:opacity-60"><span className="material-symbols-outlined mr-1 align-[-3px] text-[16px]">play_arrow</span>{isRunningCode ? 'Running…' : 'Run'}</button><button onClick={toggleCodeFullscreen} title={isCodeFullscreen ? 'Exit fullscreen' : 'Fullscreen coding'} className="grid size-8 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 hover:text-indigo-700 dark:text-secondary dark:hover:bg-surface-container"><span className="material-symbols-outlined text-[18px]">{isCodeFullscreen ? 'fullscreen_exit' : 'fullscreen'}</span></button></>}<button onClick={() => saveDocument(selectedDocument)} disabled={isSaving} className="rounded-lg px-3 py-1.5 text-xs font-bold text-indigo-700 hover:bg-indigo-50 disabled:opacity-60 dark:text-primary dark:hover:bg-primary/10"><span className="material-symbols-outlined mr-1 align-[-3px] text-[16px]">save</span>{isSaving ? 'Saving…' : 'Save'}</button><button onClick={downloadSource} title="Download source" className="grid size-8 place-items-center rounded-lg text-slate-500 hover:bg-slate-100 hover:text-slate-900 dark:text-secondary dark:hover:bg-surface-container"><span className="material-symbols-outlined text-[18px]">download</span></button><button onClick={deleteDocument} title="Delete document" className="grid size-8 place-items-center rounded-lg text-slate-400 hover:bg-red-50 hover:text-red-600 dark:text-secondary dark:hover:bg-error-container/30"><span className="material-symbols-outlined text-[18px]">delete</span></button></div></div>
               ...
               {selectedDocument.kind === 'paper' && <div className="paper-workspace-modebar flex shrink-0 items-center justify-between gap-3 border-b border-slate-200 bg-slate-50 px-4 py-1.5 text-[11px] dark:border-outline-variant/30 dark:bg-surface-container-low"><span className="inline-flex min-w-0 items-center gap-1.5 font-semibold text-slate-500 dark:text-secondary"><span className="material-symbols-outlined text-[15px] text-indigo-600 dark:text-primary">swap_horiz</span><span className="truncate">Source and PDF scroll independently. Drag the divider to resize.</span></span><button type="button" onClick={togglePaperFocusMode} aria-pressed={isPaperFocusMode} className="inline-flex shrink-0 items-center gap-1.5 rounded-md px-2 py-1 font-bold text-indigo-700 transition hover:bg-indigo-100 dark:text-primary dark:hover:bg-primary/10" title={isPaperFocusMode ? 'Exit full paper workspace (Esc)' : 'Open full paper workspace'}><span className="material-symbols-outlined text-[16px]">{isPaperFocusMode ? 'fullscreen_exit' : 'fullscreen'}</span><span className="hidden sm:inline">{isPaperFocusMode ? 'Exit focus' : 'Full workspace'}</span></button></div>}
               <div ref={selectedDocument.kind === 'paper' ? paperWorkspaceRef : null} className={selectedDocument.kind === 'paper' ? 'paper-workspace' : 'flex min-h-[calc(100vh-290px)] flex-col overflow-hidden'} style={selectedDocument.kind === 'paper' ? { '--paper-source-width': `${splitRatio}%` } : undefined}>
                <section className={selectedDocument.kind === 'paper' ? 'paper-workspace__pane paper-workspace__source bg-[#1e1e1e]' : 'min-w-0 w-full bg-[#1e1e1e]'}>
