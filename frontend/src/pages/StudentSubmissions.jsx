...
const StudentSubmissions = () => {
  ...
  const loadSubmissions = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [submissionResponse, taskResponse] = await Promise.all([
  ...
  const handleSubmit = async (event) => {
    ...
    if (!title) {
      setError("Give this deliverable a clear title.");
      return;
    }
    if (!newSub.task) {
      setError("Choose the in-progress task this deliverable satisfies.");
      return;
    }
    if (!newSub.file && !content) {
      setError(
        "Attach a file or paste the submitted text before sending this deliverable.",
      );
      return;
    }
  ...
  if (!activeProject) {
    return (
      <div className="w-full min-h-[calc(100vh-80px)] flex items-center justify-center bg-background p-6">
        <div className="max-w-lg rounded-[32px] border border-outline-variant/30 bg-surface p-12 text-center shadow-xl">
          <span className="material-symbols-outlined text-[40px] text-primary">
            upload_file
          </span>
          <h2 className="mt-5 font-display text-[28px] font-black text-on-surface">
            No Project Selected
          </h2>
          <p className="mt-2 text-secondary">
            Select a project from the top navigation to view its deliverables.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-[calc(100vh-80px)] bg-background px-5 py-7 md:px-10 md:py-10">
      <motion.div
        ...
        className="mx-auto flex max-w-[1440px] flex-col gap-7"
      >
        <motion.header
        ...
        >
          <div>
            <span className="inline-block rounded-full border border-primary/20 bg-primary/10 px-3 py-1 text-[12px] font-bold uppercase tracking-wide text-primary">
              Deliverables
            </span>
            <h1 className="mt-3 font-display text-[32px] font-black leading-none tracking-tight text-on-surface md:text-[42px]">
              {isStudent ? "My submissions" : "Deliverable review"}
            </h1>
            <p className="mt-2 text-[16px] font-medium text-on-surface-variant">
              {isStudent
                ? "Submit work and follow feedback from your assigned supervisor."
                : `Review deliverables for ${activeProject.title}, record feedback, and return work for revision when needed.`}
            </p>
          </div>
          {isStudent && (
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-2xl bg-primary px-6 py-3.5 text-[15px] font-bold text-on-primary shadow-sm transition hover:-translate-y-0.5 hover:shadow-md"
            >
        ...
        {error && (
          <div
            role="alert"
            className="rounded-2xl border border-error/30 bg-error/10 px-5 py-3 text-sm font-medium text-error"
          >
            {error}
          </div>
        )}
        {notice && (
          <div
            role="status"
            className="rounded-2xl border border-primary/30 bg-primary/10 px-5 py-3 text-sm font-medium text-on-surface"
          >
            {notice}
          </div>
        )}
        ...
        {loading ? (
          ...
          <motion.div
            variants={itemVariants}
            className="rounded-[32px] border border-outline-variant/30 bg-surface p-16 text-center shadow-sm"
          >
            ...
            <p className="mt-1 text-sm text-outline">
              {isStudent
                ? "Submit the first deliverable when it is ready for review."
                : "Student deliverables will appear here for review."}
            </p>
          </motion.div>
        ) : (
          <motion.div
            ...
            {submissions.map((submission) => {
              ...
              return (
                <motion.article
                  key={submission._id}
                  variants={itemVariants}
                  className="flex min-h-80 flex-col rounded-[28px] border border-outline-variant/30 bg-surface p-6 shadow-sm"
                >
                  <div className="flex items-start justify-between gap-3">
                    <span
                      className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-[11px] font-bold uppercase tracking-widest ${meta.tone}`}
                    >
                  ...
                  <h2 className="mt-6 text-xl font-bold leading-tight text-on-surface">
                    {submission.title}
                  </h2>
                  <p className="mt-2 text-xs font-bold uppercase tracking-wider text-primary">
                    Task:{" "}
                    {submission.task?.title || "Legacy unlinked deliverable"}
                  </p>
                  {canReview && (
                  ...
                  {submission.feedback && (
                    <div className="mt-4 rounded-xl border-l-4 border-l-tertiary bg-tertiary-container/10 p-4 text-sm text-on-surface">
                  ...
                  <div className="mt-auto flex flex-wrap items-center justify-between gap-3 border-t border-outline-variant/30 pt-5">
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-secondary">
                        Outcome
                      </p>
                      <p
                        className={`mt-1 text-sm font-bold ${submission.grade ? "text-primary" : "text-on-surface-variant"}`}
                      >
                        {submission.grade
                          ? `Grade: ${submission.grade}`
                    ...
                    <div className="flex flex-wrap justify-end gap-2">
                      {canReview && (
                        <button
                          type="button"
                          disabled={
                            String(submission.content || "").trim().length < 200
                          }
                          title={
                            String(submission.content || "").trim().length < 200
                              ? "Paste at least 200 characters of submitted text first"
                              : "Open this submission in the plagiarism checker"
                          }
                          onClick={() =>
                            navigate(
                              `/plagiarism-checker?submission=${submission._id}`,
                            )
                          }
                          className="rounded-xl border border-tertiary/30 px-3 py-2 text-xs font-bold text-tertiary transition hover:bg-tertiary/10 disabled:cursor-not-allowed disabled:opacity-45"
                        >
                          Plagiarism check
                        </button>
                      )}
        ...
        {showSubmitModal && (
          <div className="fixed inset-0 z-50 grid place-items-center bg-slate-950/45 p-4 backdrop-blur-sm">
            <motion.div
              ...
              className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-[30px] border border-outline-variant/30 bg-surface p-7 shadow-2xl"
            >
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-secondary">
                    Attach a file, paste the submitted text, or provide both.
                  </p>
                </div>
                <button
                  type="button"
              ...
              <form onSubmit={handleSubmit} className="mt-6 space-y-5">
                <label className="block text-sm font-bold text-on-surface">
                  ...
                  <select
                    ...
                    value={newSub.task}
                    onChange={(event) =>
                      setNewSub({ ...newSub, task: event.target.value })
                    }
                    className="mt-2 w-full rounded-xl border border-outline-variant/50 bg-surface-container-lowest px-4 py-3 font-normal outline-none focus:border-primary"
                  >
                    <option value="">Choose an in-progress task</option>
                  ...
                  {eligibleTasks.length === 0 && (
                    <span className="mt-2 block text-xs font-normal text-error">
                      Start an assigned task in Work before submitting its
                      deliverable.
                    </span>
                  )}
                </label>
                <label className="block text-sm font-bold text-on-surface">
                  ...
                  <input
                    ...
                    value={newSub.title}
                    onChange={(event) =>
                      setNewSub({ ...newSub, title: event.target.value })
                    }
                    className="mt-2 w-full rounded-xl border border-outline-variant/50 bg-surface-container-lowest px-4 py-3 font-normal outline-none focus:border-primary"
                ...
                <label className="block text-sm font-bold text-on-surface">
                  ...
                  <span className="font-normal text-secondary">
                    (optional when text is provided)
                  </span>
                  <input
                    type="file"
                    onChange={(event) =>
                    ...
                    className="mt-2 block w-full rounded-xl border border-outline-variant/50 bg-surface-container-lowest px-4 py-3 text-sm font-normal file:mr-4 file:rounded-full file:border-0 file:bg-primary/10 file:px-3 file:py-2 file:font-semibold file:text-primary"
                  />
                </label>
                <p className="rounded-xl border border-tertiary/25 bg-tertiary/10 p-3 text-xs leading-relaxed text-on-surface-variant">
                  On a serverless deployment, uploaded files need configured
                  object storage. If file upload is unavailable, paste the
                  submitted text below so your supervisor can still receive and
                  review it.
                </p>
                <label className="block text-sm font-bold text-on-surface">
                  Submitted text{" "}
                  ...
                  <textarea
                    ...
                    onChange={(event) =>
                      setNewSub({ ...newSub, content: event.target.value })
                    }
                    maxLength="60000"
                    rows="7"
                    className="mt-2 w-full resize-y rounded-xl border border-outline-variant/50 bg-surface-container-lowest px-4 py-3 font-normal outline-none focus:border-primary"
                    placeholder="Paste the content to submit it as text and enable the integrity screen when applicable."
                  />
                </label>
                <p className="text-xs text-secondary">
                  Integrity screening needs at least 200 characters and is an
                  aid for human review, not a plagiarism verdict.
                </p>
