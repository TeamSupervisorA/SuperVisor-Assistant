...
const ProgressLogs = () => {
  ...
  const load = async () => {
    ...
    try {
      const response = await apiFetch(`/api/projects/${activeProject._id}/progress-logs`);
  ...
  return <div className="p-6 md:p-10 max-w-5xl mx-auto space-y-8">
    ...
    <section className="space-y-4"><h2 className="text-xl font-bold">Submitted and draft logs</h2>{logs.length === 0 ? <p className="text-secondary">No logs yet.</p> : logs.map((log) => <article key={log._id} className="rounded-2xl border border-outline-variant/30 bg-surface p-5 space-y-2"><div className="flex justify-between gap-4"><strong>{new Date(log.weekStart).toLocaleDateString()} · {log.author?.name || 'You'}</strong><span className="capitalize text-secondary">{log.state}</span></div><p className="whitespace-pre-wrap">{log.summary}</p>{log.blockers && <p className="rounded-lg bg-tertiary-container/20 p-3"><strong>Blockers:</strong> {log.blockers}</p>}{user?.id === log.author?._id && log.state === 'draft' && <button onClick={() => submit(log._id)} className="rounded-lg border border-primary text-primary px-4 py-2 font-semibold">Submit log</button>}</article>)}</section>
  </div>;
};
