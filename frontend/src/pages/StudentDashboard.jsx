...
const StudentDashboard = () => {
  ...
  const openNextAction = () => {
    const action = metrics?.nextAction;
    if (!action) return currentProject ? chooseProject(currentProject) : navigate('/create-new-work');
    const project = projects.find((item) => String(item._id) === String(action.projectId));
  ...
  return <main className="min-h-[calc(100vh-80px)] bg-background p-5 md:p-8 lg:p-10">
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <section className="rounded-2xl border border-primary/20 bg-primary/5 p-5"><div className="flex flex-col justify-between gap-4 md:flex-row md:items-center"><div><p className="text-xs font-bold uppercase tracking-wider text-primary">Next action</p><h2 className="mt-1 text-xl font-extrabold text-on-surface">{loading ? 'Loading your work…' : metrics?.nextAction?.title || 'Create your first project'}</h2><p className="mt-1 text-sm text-secondary">{metrics?.nextAction?.detail || 'Start with a project proposal, then add project members and connect a supervisor.'}</p></div><button onClick={openNextAction} className="shrink-0 rounded-xl bg-primary px-4 py-2.5 text-sm font-bold text-on-primary">Open next step</button></div></section>
